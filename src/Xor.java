...
public class Xor extends BinaryExpression implements Expression {
    /**
     * The function constructs a new xor expression.
     *
    ...
    private Expression tempSimplify(Expression firstExpression,
        ...
        if (firstExpression.toString().equals(secondExpression.toString())) {
            ...
            /* That is, the first expression is T or F and the second includes
             variables.*/
            if (firstExpression.toString().equals("T")) { // The first is T
                return new Not(secondExpression); // (T ^ x) = ~(x)
            }
            return secondExpression; // (F ^ x) = x
        } else if (firstExpression.getVariables().size() > 0
                && secondExpression.getVariables().size() == 0) {
            /* That is, the second expression is T or F and the first includes
             variables.*/
            if (secondExpression.toString().equals("T")) { // The second is T
                return new Not(firstExpression); // (x ^ T) = ~(x)
            }
            return firstExpression; // (x ^ F) = x
        } else if (firstExpression.getVariables().size() == 0
                && secondExpression.getVariables().size() == 0) {
    ...
    public Boolean evaluate(Map<String, Boolean> assignment) throws Exception {
        try {
            // Return true if only one of the expressions is correct.
    ...
    public Expression assign(String var, Expression expression) {
        Xor xor = new Xor(null, null);
        return super.assignByType(var, expression, xor);
    }
    ...
    public Expression nandify() {
        ...
        if (e1 == null && e2 == null) {
            part = new Nand(super.getFirstExpression(),
                    super.getSecondExpression()); // (A NAND B)
            // A NAND (A NAND B)
            firstPart = new Nand(super.getFirstExpression(), part);
            // B NAND (A NAND B)
            secondPart = new Nand(super.getSecondExpression(), part);
        } else {
            part = new Nand(e1, e2); // (A NAND B)
            firstPart = new Nand(e1, part); // A NAND (A NAND B)
            secondPart = new Nand(e2, part); // B NAND (A NAND B)
        }

        // The calculation is: for A and B, return
        // (A NAND (A NAND B)) NAND (B NAND (A NAND B))
        return new Nand(firstPart, secondPart);
    }
    ...
    public Expression norify() {
        Expression e1 = null, e2 = null, firstPart, secondPart, rightPart, leftPart;
        ...
        if (e1 == null && e2 == null) {
            ...
            rightPart = new Nor(super.getFirstExpression(),
                    super.getSecondExpression()); // (A NOR B)
        } else {
            firstPart = new Nor(e1, e1); // (A NOR A)
            secondPart = new Nor(e2, e2); // (B NOR B)
            rightPart = new Nor(e1, e2); // (A NOR B)
        }

        leftPart = new Nor(firstPart, secondPart); // (A NOR A) NOR (B NOR B)
        // The calculation is: for A and B, return
